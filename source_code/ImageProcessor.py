...
def overlapImages(back, overlay):
    result = back
...
def quantization(image, tonesQuantity):
    result = image
    height, width, channels = image.shape
    for row in range(0,height):
        for column in range(0,width - 1):
...
def quantizationNoAlpha(image, tonesQuantity):
    result = image
    height, width, channels = image.shape
    for row in range(0,height):
        for column in range(0,width - 1):
        ...
        for column in range(0,width):
            ...
            if pixel[3] !=0:
                for c in range(0,channels):
                    ...
                    if newValue <= 255:
                        if newValue >= 0:
                            img.itemset((row,column,c), newValue)
                        else:
                            img.itemset((row,column,c), 0)
                    else:
                        img.itemset((row,column,c), 255)
    return img
